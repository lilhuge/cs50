...
int check_arg_exists(int argc);
int check_key_length(int key_len);


int main(int argc, string argv[])
{
    check_arg_exists(argc);             // Checks that a single argument exists.

    string key = argv[1];
    int key_len = strlen(key);
    string ref = "abcdefghijklmnopqrstuvwxyz";  // Creates reference alphabet to map against key.

    check_key_length(key_len);          //  Checks that key length = 26.
    ...
    for (int i = 0; i < key_len; i++)           // Calculates difference between reference and key strings
    {
        ...
        for (int j = 0; j < i ; j++)
        {
            if (key[i] == key[j])               // Checks key for repeated characters.
            {
    ...
    int n = strlen(plain);
    string cipher = plain;                      // Initialised ciphertext string
